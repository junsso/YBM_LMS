$(function(){
    ...
    $('[data-tab]').on('click', function () {
        ...
        $(this).next().find("[data-sub]").eq(0).trigger('click');

    });
    $('[data-sub]').on('click', function () {
        var tab_id = $(this).data('sub');
        $(this).parent('li').addClass('active').siblings().removeClass('active');
        $("#"+tab_id).addClass('active').siblings().removeClass('active');
        new WOW().init();

        var url = $("#"+tab_id).find('.img-wrap img').attr("src").split('?');
        $("#"+tab_id).find('img').attr("src", url[0] + '?v='+ new Date().getTime());
