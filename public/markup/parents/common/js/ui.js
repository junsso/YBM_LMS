...
$(gnbLeft.last()).on('click', function () {
  $('.gnb-inner').removeClass('on');
});
...
//스터디 보드 - 배너
var bestSwiper = new Swiper('.child-banner .swiper-container', {
  slidesPerView: 2,
  pagination: {
    el: ".child-banner .swiper-pagination",
    type: "fraction",
  },
  navigation: {
    nextEl: ".child-banner .swiper-button-next",
    prevEl: ".child-banner .swiper-button-prev",
  },
...
//스터디보드 - 학급초대 url 토글
var classInviteBtn = $('.cb-main .toggle-btns');
classInviteBtn.on('click', function () {
  $(this).parents('.cb-main').toggleClass('active');
  $('.cb-tbox').slideToggle();
  $('.class-contain .class-board-bg').toggleClass('active');
});
...
$(csSlideBtn).on('click', function () {
  ...
  if (csSlide0.hasClass('swiper-slide-active')) {
    // 학생위치
    csStudent.css('left', '0');
    // 학생 말풍선
    csStudent.children().removeClass('on');
    csStudent.children().first().addClass('on');
  }

  //1교시
  else if (csSlide1.hasClass('swiper-slide-active')) {
    // 학생위치
    csStudent.css('left', '60px');
    // 학생 말풍선
    csStudent.children().removeClass('on');
    csStudent.children().siblings('i:nth-child(2)').addClass('on');
  }

  //2교시
  else if (csSlide2.hasClass('swiper-slide-active')) {
    // 학생위치
    csStudent.css('left', '120px');
    // 학생 말풍선
    csStudent.children().removeClass('on');
    csStudent.children().siblings('i:nth-child(3)').addClass('on');
  }

  //3교시
  else if (csSlide3.hasClass('swiper-slide-active')) {
    // 학생위치
    csStudent.css('left', '180px');
    // 학생 말풍선
    csStudent.children().removeClass('on');
    csStudent.children().siblings('i:nth-child(4)').addClass('on');
  }
...
learnMoreBtn.on('click',function(){
  $(this).siblings('.learning-more-box').slideToggle();
  $(this).toggleClass('on');
  // if($(this).hasClass('on')){
  //   $('.cs-main.cs-main-more').css('margin-bottom', '100px');
  // }else {
  //   $('.cs-main.cs-main-more').css('margin-bottom', '300px');
  // }
})
...
//resize 될때 1:1메시지 선생님 리스트
$(window).resize(function(){
	var w = $(window).width();	

	if(w <= 761){
    var totalWidth = 0;
    var set = $('.tch-list .inn .item');   
    set.each(function(){
      totalWidth = totalWidth + $(this).outerWidth(true);
    })
    console.log(totalWidth);    
    $('.tch-list .inn').css('width', totalWidth);
    }
});
